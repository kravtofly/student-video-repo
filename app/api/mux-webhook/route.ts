...
export async function POST(req: NextRequest) {
  try {
    ...
    async function writeByUploadOrAsset(params: {
      ...
      const {
        uploadId, assetId, status, filename,
        ownerId, ownerName, ownerEmail,
        coachId, coachRef,
        playbackId, labId, weekNumber, level, disciplines, kind,
        reviewOrderId,
      } = params;

      // 1) Try to update the row we created at /create-upload (by upload_id)
      if (uploadId) {
        const { data: updated, error: upErr } = await supabaseAdmin
          ...
          .update({
            ...
            coach_id: asUUID(coachId ?? undefined),
            coach_ref: coachRef ?? undefined,
            ...
            review_order_id: asUUID(reviewOrderId ?? undefined),
          })
          .eq("upload_id", uploadId)
          .select("id");

        if (!upErr && updated && updated.length > 0) return true;
        if (upErr) console.error("update by upload_id error:", upErr);
      }

      // 2) No match — upsert keyed by asset_id
    ...
    return ok({ ok: true, note: "handled with error" }); // let Mux stop retrying
  }
}
